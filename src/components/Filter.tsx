...
type FilterProps = {
  tipoSelecionado: PointsType[],
  setTipoSelecionado: (novo: PointsType[]) => void,
  praiasSelecionadas: string[],
  setPraiasSelecionadas: (novo: string[]) => void,
...
export function Filter({ 
  ...
  useEffect(() => {
    onFilterChange(locaisFiltrados);
  }, [tipoSelecionado, praiasSelecionadas]);
  ...
  return (
    <>
      ...
      bg-white rounded-lg px-3 py-4 text-[14px] sm:[640px] mb-4">
        ...
        onClick={() =>
                toggleFiltro("Moitas", praiasSelecionadas, setPraiasSelecionadas)
              }>
          <span>Moitas</span>
          <div className="flex mx-auto justify-center items-center w-10 h-10 bg-[#4D658A] rounded-[50%]">
    ...
    bg-white rounded-lg px-3 py-4 text-[14px] sm:[640px]">
        <div className={`flex flex-col-reverse gap-3 items-center w-full cursor-pointer rounded-lg border-2 p-2 ${
          ...
          "border-[#010C5A]" : "border-transparent"
        }`}
        onClick={() =>
